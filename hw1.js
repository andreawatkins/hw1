...
const Product = class Product {
    ...
    constructor(data){
        const dataArr = data.split(",", 3)
        this.name = dataArr[0];
        this.price = Number(dataArr[1]);
        this.availability = dataArr[2];
    ...
    static halfOff = (products) => {
        let halfOff = [];
        let i = -1;
        halfOff = JSON.parse(JSON.stringify(products));

        for (i = 0; i < halfOff.length; i++) {
            halfOff[i].price = halfOff[i].price/2;
        }
    ...
    static printProducts = (products) => {
        ...
        while(i<products.length){
            console.log("Product: " + products[i].name + ", Cost: " + new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(products[i].price) + ", Availability: " + `${isAvailable(products[i])}`);
            i++; 
        }
